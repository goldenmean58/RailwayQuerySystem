...
Route::Route(const char *trainName, const char *srcName, const char *destName, int startTime, int arriveTime, int fee):startTime(startTime), arriveTime(arriveTime), fee(fee){
...
SubResult::SubResult(const char *trainName, const char *srcName, const char *destName, int startTime,int arriveTime,int fee):Route(trainName,srcName,destName,startTime,arriveTime,fee){
...
void Result::addSubResult(const char *trainName, const char *srcName, const char *destName, int startTime, int arriveTime, int fee, enum Condition queryCondition){
    SubResult *tmpSubResult=new SubResult(trainName,srcName,destName,startTime,arriveTime,fee);
    int timeCost=0;
    //TODO 跨日时间计算有问题
    if(startTime<lastSubResult->arriveTime && headSubResult!=lastSubResult){ //越过一天坐车
        timeCost=24+arriveTime-lastSubResult->startTime;
    }else{
        timeCost=arriveTime-(headSubResult!=lastSubResult?lastSubResult->arriveTime:startTime);
    }
...
void Result::addSubResult(Route *route){
    this->addSubResult(route->trainName,route->srcName,route->destName,route->startTime,route->arriveTime,route->fee,route->queryCondition);
}
...
bool Backend::addRoute(const char *trainName, const char *srcName,const char *destName, int startTime, int arriveTime, float feeCost){
    //不允许出现自环
    ...
    if(srcId<0 || destId<0) //未知城市
        return false;
    Route *newRoute=new Route(trainName,srcName,destName,startTime,arriveTime,feeCost);
...
bool Backend::modRoute(const char *trainName, const char *srcName,const char *destName, int startTime, int arriveTime, float feeCost){
    return this->removeRoute(trainName) && 
            this->addRoute(trainName,srcName,destName,startTime,arriveTime,feeCost);
}
...
Result *Backend::getBestResult(const char *srcName, const char *destName, enum Condition queryCondition){
...
vector<Result*> Backend::getAllSortedResult(const char *srcName, const char *destName, enum Condition queryCondition){
    vector<Result*> ret;
    Result *route=NULL;
    int src=cityName2id(srcName), dest=cityName2id(destName);
    if(src>=0 && dest>=0){
