...
void outputResult(Result * ret)
{
	...
	if (ret == NULL) {
		...
		cout << "Total Time:" << ret->getTotalTimeCost()
				...
				<< '\t' << "Total Transit:" << ret->getTotalStationNum() - 1 << endl;
	}
	delete ret;
}
...
int main(void)
{
	...
	while (true) {
		...
		case 4:
			...
			cin >> fee;
			if (backend.
					addRoute(trainName, srcName, destName, startTime, arriveTime, fee)) {
				cout << "添加车次成功!" << endl;
			} else {
				cout << "添加车次失败!" << endl;
			}
		...
		case 5:
			...
			cin >> fee;
			if (backend.
					modRoute(trainName, srcName, destName, startTime, arriveTime, fee)) {
				cout << "修改车次成功!" << endl;
			} else {
				cout << "修改车次失败!" << endl;
			}
		...
		case 6:
			...
			break;
		case 7:
			cout << setw(15) << "Train Name" << setw(15) << "Start Station" <<
					setw(15) << "End Station" << setw(15) << "Start Time" << setw(15) <<
					"Arrive Time" << setw(15) << "Ticket Fee" << endl;
			for (int i = 0; i < backend.m_nNumCity; i++) {
				for (int j = 0; j < backend.m_nNumCity; j++) {
					p = backend.m_aMatrix[i][j]->next;
					while (p != NULL) {
						cout << setw(15) << p->trainName << setw(15) << p->
								srcName << setw(15) << p->destName << setw(15) << p->
								startTime << setw(15) << p->arriveTime << setw(15) << p->
								fee << endl;
						p = p->next;
					}
				}
		...
		case 8:
			...
			cin >> destName;
			cout <<
					"请选择决策(1.最短时间 2.最少旅费 3.中转最少 4.所有最佳方案):";
			cin >> condition;
			switch (condition) {
			case 1:
